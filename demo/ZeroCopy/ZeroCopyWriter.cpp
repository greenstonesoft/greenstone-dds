    ...
    const bool& wait)
{
    ...
    {
        void* ptr = nullptr;
        m_writer->loan_sample(ptr, sizeof(Helloworld));
        m_helloworld = static_cast<Helloworld*>(ptr);
        ...
        memcpy(m_helloworld->message().data(), text.c_str(), text.size() + 1);
        ...
        if (m_writer->write(m_helloworld, m_handle) == greenstone::dds::ReturnCode_t::RETCODE_OK)
        {
            std::cout << "[ID: " << m_helloworld->id() 
                      ...
                      << "; Message size: " << strlen(m_helloworld->message().data()) 
                      << " B] SENT" << std::endl;
        }
        else
        {
