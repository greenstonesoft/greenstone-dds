...
bool WaitSetReader::init(const std::string& topicName)
{
    ...
    std::string topic_type_name = m_waitSetTopicType.get_name();
...
{    
    ...
    {    
        ...
        if (statusChanges & greenstone::dds::StatusKind::DATA_AVAILABLE_STATUS)
        {
            ...
            if (m_reader->take_next_sample(&waitSet, info) == greenstone::dds::ReturnCode_t::RETCODE_OK)
            {
                if (info.valid_data)
                {
                    ++received;
                    std::cout << "[ID: " << waitSet.id() 
                              << "; Index: " << waitSet.index() 
                              << "; Received: " << received
                              << "; Message size: " << waitSet.message().size() 
                              << " B] RECEIVED" << std::endl;
                }
            }
        }
