...
#include "swiftdds/rtps/ErrorInfo.h"
...
public:
	...
	*/
	...
	/**
	 * @brief init the Buf object with param in.
	 * @param [in] buf buffer address.
	 * @param [in] len buffer length.
	 * @note no exception.
	*/
	gvoid_t init(gvoid_t * const buf, guint32_t const len) noexcept;
	...
	*/
	...
	/**
	 * @brief Offset the buffer's current length.
	 * @param [in] count the offset size.
	*/
	gvoid_t length_offset(guint32_t const count) noexcept;
	...
	*/
	 ...
	 * @param [in] v_pos the position's value.
	 * @note no exception.
	*/
	gvoid_t set_pos(guint32_t const v_pos) noexcept
	{
		m_nPos = v_pos;
	}

	/**
	 * @brief get the buffer's position.
	 * @return guint32_t 
	 * @retval the real pos value.
	 * @note no exception.
	*/
	guint32_t pos() const noexcept
	{
		return m_nPos;
	}
