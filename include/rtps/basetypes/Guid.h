...
public:
     ...
     */
    GUID(rtps::GuidPrefix_t const &prefix, rtps::EntityId_t const &entity_id)
    noexcept : m_guidPrefix {prefix}, m_entityId {entity_id}
    {
    }
    ...
    void from_octet16(octet const oct[GUID_SIZE]) noexcept
    {
        ...
        std::array<octet, KEY_SIZE> const key {
            *static_cast<std::array<octet, KEY_SIZE> const *>(
...
struct hash<gstone::rtps::GUID> final
{
        ...
        noexcept(noexcept(std::string(std::declval<gchar_t const *>())))
    {
        std::string const guidPrefix {
            static_cast<gchar_t const *>(static_cast<const void *>(obj.guid_prefix().value().data()))};
        std::string const entityKey {
            static_cast<gchar_t const *>(static_cast<const void *>(obj.entity_id().entity_key().data()))};

        return hash<string>()(guidPrefix) ^ hash<string>()(entityKey);
    }
};
