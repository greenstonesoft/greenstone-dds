...
public:
     ...
     * @param [in] _is_decode
     */
    inline void is_decode(bool const _is_decode) noexcept
    {
        m_isDecode = _is_decode;
    }
     ...
     * @param [in] is_new
     */
    inline void is_new(bool const is_new) noexcept
    {
        m_isNew = is_new;
    }
private:
    inline SerializedPayload(guint32_t const _length, octet *const _value, bool const _need_free,
                             bool const _is_zero_copy, bool const _is_decode) noexcept;
